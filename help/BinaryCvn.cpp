...
QString BinaryCvn::byteArrayToAsciiStr(QByteArray data)//ascii码接受
{
    QString temp;
...
QByteArray BinaryCvn::hexStrToByteArray(QString str)//16进制发送
{
    QByteArray senddata;
...
QByteArray BinaryCvn::asciiStrToByteArray(QString str)//ascii发送
{
    QByteArray buffer;
    int len = str.length();
    QString letter;
...
char BinaryCvn::convertHexChar(char ch)
{
    if ((ch >= '0') && (ch <= '9')) {
        return ch - 0x30;
    } else if ((ch >= 'A') && (ch <= 'F')) {
...
QString BinaryCvn::decimalToStrHex(int decimal)//十进制转换16进制
{
...
int BinaryCvn::strHexToDecimal(QString strHex)
{
    bool ok;
